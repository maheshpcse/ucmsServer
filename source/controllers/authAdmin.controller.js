...
const adminLogin = async (request, response, next) => {
    ...
    try {
        await userSP.selectDataSP(spConfig.GET_ADMIN_LOGIN, [request.body.adminLoginName], null).then(async resData => {
...
const getAdminAndSettingsLogout = async (request, response, next) => {
    ...
    try {
        await userSP.insertOrUpdateDataSP(spConfig.GET_ADMIN_AND_SETTINGS_LOGOUT, [request.body.user_info_id], null).then(async resData => {
